...
const ContainerCss = ({ displayCount, slideAnchors }) => {
  const minWidth = calcMinWidth(slideAnchors)
...
export const Carousel = ({
  ...
  const onTouchStart = useCallback(
    (e) => {
      ...
      if (areArrowsLocked.current || !isDraggable || e.touches?.length > 1) {
        return
      }
  ...
  const onTouchMove = useCallback(
    ...
    [isDraggable, isDragging, setTranslateOffset],
  )
  ...
  const onScroll = useCallback(
    (e) => {
      if (areArrowsLocked.current || isDragging) {
        return
      }
  ...
  const onSlideFocus = useCallback(
    (i) => (e) => {
      ...
      if (isFirstFocus && i >= minTabIndex && i <= maxTabIndex) {
        ...
        const isEnabled = !areArrowsLocked.current && !(isDraggable && isDragging)

        if (isEnabled && (i < index.left || i > index.right)) {
          onArrowClick((i < index.left ? -1 : 1) * (isInfinite ? 1 : activeIndexes.length), () => {
            //failsafe to prevent forced scroll when focusing elements
            slidesOuterContainerRef.current.scrollLeft = 0
  ...
  useEffect(() => {
    if (!areArrowsLocked.current && !(isDraggable && isDragging)) {
      ...
      //focus correct child after scroll
  ...
  const containerCss = useMemo(
    ...
    [slideAnchors, slideAnchors?.length, displayCount],
  )
